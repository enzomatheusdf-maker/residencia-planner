    ...
    <MotionConfig reducedMotion="user">
      <AnimatePresence>
        {open && message && (
          <motion.div
            key="med-toast"
            className="fixed bottom-24 left-1/2 z-[var(--med-z-toast)] w-[min(calc(100vw-1.5rem),26rem)] -translate-x-1/2 md:bottom-6"
            initial={{ opacity: 0, y: 12 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 12 }}
            transition={{ duration: 0.22, ease: EASE_STANDARD }}
          >
            <div className={`med-card flex items-center gap-3 px-4 py-3 ${toneClasses[tone] || toneClasses.info}`}>
