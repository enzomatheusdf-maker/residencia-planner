...
function resolveActionCTA(action, onStudy, setView) {
  ...
  if (view && setView) {
    return { label: "Abrir", onClick: () => setView(view) };
  }
  return null;
}
    ...
    <section className="bg-[var(--surface-1)] border border-white/5 rounded-2xl p-4 space-y-3">
      ...
      <article className="rounded-xl border border-blue-500/20 bg-blue-500/10 p-3 space-y-2">
        ...
        <div className="flex flex-wrap gap-2 pt-1">
          {resolveActionCTA(primary, onStudy, setView) && (
            <button
              type="button"
              onClick={resolveActionCTA(primary, onStudy, setView).onClick}
            ...
            >
              <Play size={12} /> {resolveActionCTA(primary, onStudy, setView).label}
            </button>
          )}
          <button
            type="button"
