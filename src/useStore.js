...
export const ESP_COLORS = {
  "Cirurgia": "#fb923c", "Clínica Médica": "#60a5fa", "GO": "#f472b6",
  "Pediatria": "#34d399", "Preventiva": "#c084fc", "Outro": "#94a3b8",
  "Exatas": "#22d3ee", "Humanas": "#fbbf24", "Linguagens": "#a3e635",
  "Ciências da Natureza": "#2dd4bf", "Redação": "#f0abfc",
};
export const PRIO = {
  "Diamante": { c: "#38bdf8" }, "Alta": { c: "#f87171" },
  "Média":    { c: "#fbbf24" }, "Baixa": { c: "#94a3b8" }, "Bônus": { c: "#c084fc" },
};
...
export function calcFilaInteligente(temas) {
  ...
  temas.forEach(t => {
    const acertos = STEPS
      .filter(s => t.rev[s.key].done && t.rev[s.key].acerto != null)
      .map(s => t.rev[s.key].acerto);
    const acertoMedia = acertos.length ? acertos.reduce((a,b)=>a+b)/acertos.length : 0.5;
    ...
    STEPS.forEach(s => {
      ...
      const isToday = isDueToday(r.date);
      if (!overdue && !isToday) return;
      const urgencia = overdue ? 1.5 : 1.0;
      const score = (1 - acertoMedia) * peso * urgencia;
      items.push({ temaId: t.id, temaNome: t.nome, esp: t.esp, importancia: imp,
        stepKey: s.key, step: s, date: r.date, overdue, score: +score.toFixed(2) });
    });
  });
...
export function calcMetricasElite(simulados) {
  const sims = simulados.map(migrarSim);
  const todasErradas = sims.flatMap(s => s.questoesErradas || []);
  const totalErros   = todasErradas.length;
  const descuidos    = todasErradas.filter(q => q.tipoErro === "descuido").length;
  const indiceDescuido = totalErros >= 3 ? Math.round(descuidos / totalErros * 100) : null;

  const simsComTempo = sims.filter(s => s.tempoMin > 0);
  const velEficiente = simsComTempo.length >= 2
    ? +(simsComTempo.reduce((a,s) => a + s.acertos / (s.tempoMin / 60), 0) / simsComTempo.length).toFixed(1)
    : null;

  const verificadas   = todasErradas.filter(q => q.corrigidaD7 != null);
  const convertidas   = verificadas.filter(q => q.corrigidaD7 === true).length;
  const taxaConversao = verificadas.length >= 3 ? Math.round(convertidas / verificadas.length * 100) : null;

  const porArea = {};
  todasErradas.forEach(q => {
    if (!q.esp) return;
    if (!porArea[q.esp]) porArea[q.esp] = { total:0, lacuna:0, raciocinio:0, distractor:0, descuido:0, nao_visto:0 };
    porArea[q.esp].total++;
    if (q.tipoErro) porArea[q.esp][q.tipoErro] = (porArea[q.esp][q.tipoErro] || 0) + 1;
  });
  const diagnostico = Object.entries(porArea).map(([esp, v]) => {
    const tipos = ["lacuna","raciocinio","distractor","descuido","nao_visto"];
    const dominante = tipos.reduce((a,t) => (v[t]||0) > (v[a]||0) ? t : a, "lacuna");
    return { esp, ...v, dominante,
  ...
  return { indiceDescuido, velEficiente, taxaConversao, diagnostico, insights: insights.slice(0,3) };
}
...
export const MEDCOF = [
  ...
  {b:2,t:[["Neurovascular I: AIT e AVCi","Clínica Médica","Diamante"],["Neurovascular II: HSA e AVCh","Clínica Médica","Alta"],["Anatomia Pélvica Feminina","GO","Diamante"],["Embriologia do Sistema Genital Feminino","GO","Média"],["Malformações Mullerianas","GO","Média"],["Febre sem Sinais Localizatórios","Pediatria","Média"],["Nefrologia Pediátrica","Pediatria","Diamante"],["Doença Renal Crônica e Lesão Renal Aguda","Pediatria","Baixa"],["Miscelânea em Nefropediatria","Pediatria","Média"],["Testes Diagnósticos","Preventiva","Diamante"],["Assistência ao Pré-Natal na APS","Preventiva","Média"],["Trauma: Medidas Auxiliares e FAST","Cirurgia","Baixa"],["Trauma: Populações Especiais","Cirurgia","Bônus"],["Trauma de Tórax","Cirurgia","Diamante"]]},
...
export function calcStreaks(doneDays) {
  ...
  const sinceLast = diffDays(sorted[sorted.length - 1], todayStr());
...
export function calcTrueRetention(temas) {
  const vals = [];
  temas.forEach((t) => {
    STEPS.filter(s => s.offset > 15).forEach((s) => {
      const r = t.rev[s.key];
...
const initialVestibularPlat = () => {
  const temasData = [
    ...
    { id: 15, nome: "Redação - Argumentação", esp: "Linguagens", prio: "Alta", importancia: "CRITICA", d0: "2026-06-03", obs: "Foco C5: 5 elementos" },
...
export const useStore = create(
  persist(
    (set, get) => ({
      ...
      exportKey: () => {
        const s = get();
        return btoa(JSON.stringify({ userName: s.userName, plat: s.plat, meta: s.meta,
          res: s.res, vest: s.vest, onboardingDone: s.onboardingDone }));
      },
      importKey: (key) => {
        try {
          const data = JSON.parse(atob(key));
          set({ userName: data.userName, plat: data.plat, meta: data.meta,
            res: data.res, vest: data.vest, onboardingDone: data.onboardingDone });
          return true;
        } catch { return false; }
      },
      ...
      addTema: (platKey, tema) =>
        set((s) => ({ [platKey]: { ...s[platKey], temas: [...s[platKey].temas,
          { ...tema, id: Date.now(), importancia: tema.importancia || "ALTA",
            ankiDeck: tema.ankiDeck || "", rev: buildRev(tema.d0) }] } })),
      ...
      markStep: (platKey, temaId, stepKey, { acerto, questoes, motivosErro }) =>
        set((s) => ({
          [platKey]: { ...s[platKey],
            temas: s[platKey].temas.map((t) => {
              if (t.id !== temaId) return t;
              const revMarked = { ...t.rev,
                [stepKey]: { ...t.rev[stepKey], done: true, acerto, questoes, motivosErro: motivosErro || [] } };
      ...
      importTemas: (platKey, items, d0) =>
        set((s) => ({ [platKey]: { ...s[platKey], temas: [...s[platKey].temas,
          ...items.map((it) => ({ id: Date.now() + Math.random(), nome: it.nome, esp: it.esp,
            prio: it.prio, importancia: "ALTA", obs: "MEDCOF 2026", pico: "", ankiDeck: "", d0,
            rev: buildRev(d0) }))] } })),

      optimize: (platKey) =>
        set((s) => ({ [platKey]: { ...s[platKey],
          temas: s[platKey].temas.map((t) => {
            const nr = { ...t.rev };
            ...
            STEPS.forEach((step, i) => {
              ...
              else if (nr[step.key].done) lastDate = nr[step.key].date || lastDate;
            });
            return { ...t, rev: nr };
          }),
        } })),
      ...
      addSim: (platKey, sim) =>
        set((s) => ({ [platKey]: { ...s[platKey], simulados: [...s[platKey].simulados,
          { questoesErradas: [], statusCorrecao: "pendente", porArea: [], ...sim, id: Date.now() }] } })),
      ...
      addQuestaoErrada: (platKey, simId, questao) =>
        set(s => ({ [platKey]: { ...s[platKey], simulados: s[platKey].simulados.map(sim =>
          sim.id !== simId ? sim : { ...sim,
      ...
      marcarD7: (platKey, simId, questaoId, acertou) =>
        set(s => ({ [platKey]: { ...s[platKey], simulados: s[platKey].simulados.map(sim => {
          ...
          return { ...sim, questoesErradas: qes,
            statusCorrecao: qes.length > 0 && qes.every(q => q.corrigidaD7 != null) ? "concluida" : "parcial" };
        }) } })),

      updateSim: (platKey, simId, fields) =>
        set(s => ({ [platKey]: { ...s[platKey], simulados: s[platKey].simulados.map(sim =>
          sim.id !== simId ? sim : { ...sim, ...fields }) } })),

      addReflexao: (platKey, temaId, reflexao) =>
        set(s => ({ [platKey]: { ...s[platKey], temas: s[platKey].temas.map(t =>
          t.id !== temaId ? t : { ...t, reflexao: { ...reflexao, data: todayStr() } }) } })),
