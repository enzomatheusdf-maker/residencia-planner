...
export function decideMentorAction(context = {}) {
  ...
  if (shouldRecover && Number(scheduler.relearningCount || 0) > 0) {
    ...
    return buildAction({
      ...
      explain: [
        domainCopy.explain,
        item.domainTestAgendaLabel ? `Classificação do Teste de Domínio: ${item.domainTestAgendaLabel}.` : "Houve queda recente em revisão espaçada.",
  ...
  if (context.recommendedSimulation && canSuggestNewTopic(context)) {
    ...
    return buildAction({
      ...
      subtitle: rec.frequenciaRecomendada ? `Frequência: ${rec.frequenciaRecomendada}.` : "Fila segura; simulado entra como próxima ação.",
      reason: rec.justificativa || "O simulado calibra desempenho e revela gargalos para o próximo ciclo.",
      explain: [
        ...
        "Se houver fadiga alta, reduza escopo em vez de forçar prova completa.",
      ],
      cta: "Abrir simulados",
      ctaView: "sims",
  ...
  if (context.dominantErrorIsStrong && context.dominantErrorAction) {
    ...
    return buildAction({
      ...
      priority: 78,
      title: `Ação corretiva: ${da.label}`,
      subtitle: "Padrão de erro recorrente identificado.",
      reason: da.definition,
      explain: [
        da.correctiveActions[0] || "Aplique a ação corretiva recomendada.",
        da.correctiveActions[1] || null,
        da.fsrsEffect || null,
      ].filter(Boolean),
      cta: "Ver ação corretiva",
      ctaView: "stats",
      estimatedMinutes: 20,
      confidence: 0.82,
      safety: "ok",
  ...
  if (canSuggestNewTopic(context)) {
    ...
    if (canSuggestInterleavingBlock(context)) {
      return buildAction({
        ...
        subtitle: "Fila vazia; há corpus suficiente para contraste ativo.",
        reason: "Com duas ou mais áreas consolidadas, interleaving ajuda a treinar discriminação sem abrir cobertura nova.",
        explain: [
          "Fila do dia e vencidas estão zeradas.",
          `${context.consolidatedCorpus || 0} áreas já têm corpus consolidado.`,
          "Prioridade combina menor maestria com maior incidência.",
        ],
        cta: "Abrir foco",
        ctaView: "focus",
        estimatedMinutes: 35,
